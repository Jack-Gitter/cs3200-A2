...
/*!50001 VIEW `developer_roles_and_priviledges` AS select distinct `persons`.`first_name` AS `first_name`,`persons`.`last_name` AS `last_name`,`persons`.`email` AS `email`,`websites`.`name` AS `name`,`websites`.`visits` AS `visits`,`websites`.`updated` AS `website_updated`,`website_roles`.`role` AS `website_role`,`website_priviledges`.`priveledge` AS `priveledge`,`pages`.`title` AS `title`,`pages`.`views` AS `views`,`pages`.`updated` AS `page_updated`,`page_roles`.`role` AS `page_roles`,`page_priviledges`.`priviledge` AS `priviledge` from (((((((`persons` join `developers`) join `websites`) join `pages`) join `website_priviledges`) join `website_roles`) join `page_priviledges`) join `page_roles`) where ((`persons`.`id` = `developers`.`id`) and (`website_roles`.`developer` = `developers`.`id`) and (`website_roles`.`website` = `websites`.`id`) and (`page_roles`.`developer` = `developers`.`id`) and (`page_roles`.`page` = `pages`.`id`) and (`pages`.`website` = `websites`.`id`)) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2020-10-15 12:17:38
